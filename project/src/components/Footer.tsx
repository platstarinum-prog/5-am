    ...
    <footer className="border-t border-zinc-800 mt-24">
      <div className="max-w-screen-xl mx-auto px-6 py-16">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-12">
          ...
          <div>
            <p className="text-zinc-600 font-mono text-[10px] uppercase tracking-widest mb-3">{t('footer.tagline')}</p>
            ...
            <p className="text-zinc-500 text-sm mt-4 max-w-xs">{t('footer.desc')}</p>
          </div>

          <div className="flex flex-col gap-3 md:items-end">
            <p className="text-zinc-600 font-mono text-[10px] uppercase tracking-widest mb-1">{t('footer.nav')}</p>
            <a href="/catalog" className="text-zinc-400 hover:text-white text-sm transition-colors">{t('footer.catalog')}</a>
            <a href="/reviews" className="text-zinc-400 hover:text-white text-sm transition-colors">{t('footer.reviews')}</a>
            <a href="/faq" className="text-zinc-400 hover:text-white text-sm transition-colors">{t('footer.faq')}</a>
          </div>

          <div className="flex flex-col gap-3 md:items-end">
            <p className="text-zinc-600 font-mono text-[10px] uppercase tracking-widest mb-1">{t('footer.contact')}</p>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-8 border-t border-zinc-900">
          <p className="text-zinc-700 font-mono text-[10px] uppercase tracking-widest">{t('footer.copyright', { year })}</p>
          <p className="text-zinc-800 font-mono text-[10px] uppercase tracking-widest">{t('footer.est')}</p>
        </div>
